  ...
  template: `
    <div class="flex">
      <div class="relative">
        <input
          type="search"
          placeholder="{{ svpPlaceholder }}"
          class="block {{ widthClass }} z-20 text-sm rounded-e-lg"
          [(ngModel)]="searchTerm"
        ...
        <button
          type="submit"
          class="absolute end-0 top-0 h-full rounded-e-lg border border-primary-700 bg-primary-700 p-2 text-sm font-medium text-white focus:outline-none focus:ring-4 focus:ring-transparent hover:bg-primary-800 dark:border-night-500 dark:bg-night-500 dark:hover:bg-night-500"
...
export class SvpSearchButtonComponent {
  ...
  handleOnKeyUp(e: KeyboardEvent): void {
    ...
    if (this.searchTerm.length != 0 && this.searchTerm.length < this.minLetters) return;
